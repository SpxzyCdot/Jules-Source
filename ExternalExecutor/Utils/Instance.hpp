...
public:
    ...
    std::string GetScriptBytecode() {
        ...
        uintptr_t bytecode_ptr = ReadMemory<uintptr_t>(embedded + 0x10, ProcessID);
        uint64_t size = ReadMemory<uint64_t>(embedded + 0x20, ProcessID);

        std::string bytecode;
        bytecode.resize(size);

        Memory::ReadNative(bytecode_ptr, &bytecode[0], size, ProcessID);
        return bytecode;
    }
